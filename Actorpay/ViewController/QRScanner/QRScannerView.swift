...
public class QRCodeScannerController: UIViewController, AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationBarDelegate {
    ...
    var squareView: QRSquareView? = nil
    ...
    //Default Properties
    private let bottomSpace: CGFloat = 0.0//80.0
    private let spaceFactor: CGFloat = 0.0//16.0
    ...
    func createCornerFrame() {
        ...
        let rect = CGRect.init(
            origin: CGPoint.init(
                ...
                y: (self.view.frame.height - width)/2-((ScreenSize.SCREEN_MAX_LENGTH >= 812.0) ? 250 : 125)),
    ...
    func addMaskLayerToVideoPreviewLayerAndAddText(rect: CGRect) {
        ...
        noteText.fontSize = 14.0
        noteText.string = "Scan the given QR code to make\na direct payment "
        noteText.alignmentMode = CATextLayerAlignmentMode.center
        noteText.contentsScale = UIScreen.main.scale
        noteText.frame = CGRect(x: spaceFactor, y: rect.origin.y + rect.size.height + ((ScreenSize.SCREEN_MAX_LENGTH == 568.0) ? 20 : 40), width: view.frame.size.width - (2.0 * spaceFactor), height: 40)
